...
import { BandMemberRow, toBandMember} from "@/modules/band-members/infrastructure/mappers/band-member.mapper";
import { query } from "@/utils/db";

export class DbBandMemberRepository implements BandMemberRepository {
  async getAll(): Promise<BandMember[]> {
    const res = await query<BandMemberRow>(
        `SELECT * FROM band_members ORDER BY name ASC`
    );

    return res.rows.map(toBandMember);
  }

  async create(member: Omit<BandMember, "id">): Promise<BandMember> {
    const res = await query<BandMemberRow>(
        `INSERT INTO band_members (name, role, image_url)
         VALUES ($1, $2, $3) RETURNING *`,
        [member.name, member.role, member.imageUrl]
    );

    return toBandMember(res.rows[0]);
  }
